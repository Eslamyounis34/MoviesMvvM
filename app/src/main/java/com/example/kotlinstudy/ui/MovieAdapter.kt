...
class MovieAdapter(private val users: List<Movie>) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view =
            LayoutInflater.from(parent.context).inflate(R.layout.recycleritemrow, parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
      //  holder.name.setText(users[position].title)
        Picasso.get().load("https://image.tmdb.org/t/p/w500/" + users[position].poster_path)
            .into(holder.poster)
